...
export default function Wh40kCalculator() {
  ...
  const persistHistory = useCallback(async (next) => {
    setHistory(next);
    try {
      await window.storage.set("history_v1", JSON.stringify(next), false);
    } catch (e) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("library_v2", false);
    ...
    (async () => {
      try {
        const res = await window.storage.get("armies_v1", false);
    ...
    (async () => {
      try {
        const res = await window.storage.get("history_v1", false);
  ...
  const persistLibrary = useCallback(async (next) => {
    setLibrary(next);
    try {
      await window.storage.set("library_v2", JSON.stringify(next), false);
    } catch (e) {
  ...
  const persistArmies = useCallback(async (next) => {
    setArmies(next);
    try {
      await window.storage.set("armies_v1", JSON.stringify(next), false);
    } catch (e) {
